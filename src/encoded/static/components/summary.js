...
const SummaryHorizontalFacets = ({ context, facetList }, reactContext) => {
    let horizFacets;
    if (facetList === 'all') {
        horizFacets = context.facets.filter(f => ['donors.sex', 'donors.development_ontology.development_slims', 'donors.ethnicity.term_name'].includes(f.field));
    } else {
...
class SummaryBody extends React.Component {
    ...
    render() {
        ...
        const vertFacetNames = ['assay', 'protocol.title', 'observation_count', 'biosample_classification', 'biosample_ontologies.system_slims', 'biosample_ontologies.organ_slims', 'biosample_ontologies.term_name', 'award.focus', 'award.project', 'award.coordinating_pi.title', 'lab.title'];
        const vertFacets = []
